...
import type { OllamaPullEvent, OllamaStatus } from '../../types';
          ...
          <div className="mt-4 space-y-2">
            ...
            {models.length ? (
              <ul className="divide-y divide-warm-200/70 rounded-lg border border-warm-200/70 bg-white/60 dark:divide-warm-600/70 dark:border-warm-600/70 dark:bg-warm-700/30">
                {models.map((model) => (
                  <li key={model} className="flex items-center justify-between px-3 py-2">
                    <span className="text-sm text-warm-700 dark:text-warm-100">{model}</span>
                    <button
                      type="button"
